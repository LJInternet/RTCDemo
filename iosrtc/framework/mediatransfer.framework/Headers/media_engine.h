...
MEDIATRANSFER_EXTERN struct media_engine;
    ...
    MEDIATRANSFER_EXTERN typedef void (*audio_data_ex_cb)(void *audioData, int size, long pts, int sampleRate, int channelCont, long uid);

    MEDIATRANSFER_EXTERN typedef void (*video_data_ex_cb)(uint8_t* buf, int32_t len, uint8_t* frameInfo, int32_t frameInfoSize);

    MEDIATRANSFER_EXTERN typedef void (*event_cb)(int type, const char *buf, int size, void* context);

    MEDIATRANSFER_EXTERN void media_engine_set_debug_env(int debug);
    ...
    MEDIATRANSFER_EXTERN void media_engine_start_camera_capture_with_config(struct media_engine* engine, char* deviceName, const char* config, int length);
    ...
    MEDIATRANSFER_EXTERN void media_engine_unsubscribe_audio(struct media_engine* engine, int type);
    ...
    MEDIATRANSFER_EXTERN void media_engine_subscribe_video_ex(struct media_engine* engine, video_data_ex_cb cb);

    MEDIATRANSFER_EXTERN void media_engine_subscribe_audio_ex(struct media_engine* engine, audio_data_ex_cb cb);

    MEDIATRANSFER_EXTERN int media_engine_push_audio_ex(struct media_engine* engine, const int8_t *pcm, int frame_num, int sampleRate, int channelCount, int bytePerSample, const char *key);
#ifdef __cplusplus
}
#endif
