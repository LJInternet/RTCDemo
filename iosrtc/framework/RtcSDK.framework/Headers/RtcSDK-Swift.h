...
@class TransDelayInfoListWrapper;
/// Swift wrapper for CNeedDecodeVideoFrame C struct
SWIFT_CLASS("_TtC6RtcSDK28CNeedDecodeVideoFrameWrapper")
@interface CNeedDecodeVideoFrameWrapper : NSObject
@property (nonatomic) uint8_t * _Nullable buf;
@property (nonatomic) int32_t len;
@property (nonatomic) int32_t width;
@property (nonatomic) int32_t height;
@property (nonatomic) int32_t frameType;
@property (nonatomic) uint64_t pts;
@property (nonatomic) int32_t pixel_fmt;
@property (nonatomic, strong) TransDelayInfoListWrapper * _Nullable delayData;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@protocol NeedDecodeVideoFrameDelegate;
...
- (void)setupRemoteVideoWithView:(UIView * _Nullable)view renderMode:(enum RenderMode)renderMode;
- (BOOL)joinChannelWithChannelConfig:(ChannelConfig * _Nonnull)channelConfig SWIFT_WARN_UNUSED_RESULT;
/// registerDecodeVideoFrameObserver 和 registerNeedDecodeVideoFrameObserver两个方法是互斥的
/// registerDecodeVideoFrameObserver 用于注册解码视频帧的回调，默认加入频道（joinChannel）会自动调用解码，
/// 注册该回调后，registerNeedDecodeVideoFrameObserver的回调将不会生效
/// registerNeedDecodeVideoFrameObserver 用于注册需要解码视频帧的回调， 注册该回调后，registerDecodeVideoFrameObserver的回调将不会生效
/// 同时注册该回调，将不会自动调用解码，需要有用户自己解码，不存在默认解码但又需要回调不解码数据的实现
- (void)registerDecodeVideoFrameObserverWithDelegate:(id <DecodeVideoFrameDelegate> _Nonnull)delegate;
- (void)registerNeedDecodeVideoFrameObserverWithDelegate:(id <NeedDecodeVideoFrameDelegate> _Nonnull)delegate;
...
SWIFT_PROTOCOL("_TtP6RtcSDK28NeedDecodeVideoFrameDelegate_")
@protocol NeedDecodeVideoFrameDelegate
- (void)onNeedDecodeVideoFrameWithFrame:(CNeedDecodeVideoFrameWrapper * _Nonnull)frame;
@end
...
typedef SWIFT_ENUM(int32_t, RenderMode, closed) {
  RenderModeHidden = 1,
  RenderModeFit = 2,
...
/// Swift wrapper for TransDelayInfoList C struct
SWIFT_CLASS("_TtC6RtcSDK25TransDelayInfoListWrapper")
@interface TransDelayInfoListWrapper : NSObject
@property (nonatomic) void * _Nullable timeInfos;
@property (nonatomic) uint32_t size;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

/// Swift wrapper for TransDelayInfo C struct
SWIFT_CLASS("_TtC6RtcSDK21TransDelayInfoWrapper")
@interface TransDelayInfoWrapper : NSObject
@property (nonatomic) uint64_t id;
@property (nonatomic) uint64_t value;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithId:(uint64_t)id value:(uint64_t)value OBJC_DESIGNATED_INITIALIZER;
@end
...
typedef SWIFT_ENUM(int32_t, UnsubscribeType, closed) {
  ...
  UnsubscribeTypeVIDEO_CALLBACK_DECODE_EX = 54,
  UnsubscribeTypeVIDEO_CALLBACK_UNDECODE = 55,
  UnsubscribeTypeVIDEO_CALLBACK_DELAY_INFO = 56,
};
