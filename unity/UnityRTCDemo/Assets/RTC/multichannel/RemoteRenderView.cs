...
namespace LJ.RTC
{
    public class RemoteRenderView
    {
        ...
        private int mWidth;
        private int mHeight;
        public RemoteRenderView(string key)
        {
            JLog.Info("RemoteRenderView create " + key);
        ...
        public void OnDecodedVideoFrame(byte[] buffer, Int32 width,
            Int32 height, int pixel_fmt)
        {
            ...
            Buffer.BlockCopy(buffer, 0, mYBuffer, 0, ySize);
            Buffer.BlockCopy(buffer, ySize, mUBuffer, 0, uvSize);
            Buffer.BlockCopy(buffer, ySize + uvSize, mVBuffer, 0, uvSize);
            mWidth = width;
            mHeight = height;
