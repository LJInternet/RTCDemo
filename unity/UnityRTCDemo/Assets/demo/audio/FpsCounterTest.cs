...
public class FpsCounterTest
{
    ...
    public FpsCounterTest(string parentTag)
    {
        mTag = parentTag + "_" + TAG;
    }

    public FpsCounterTest(string parentTag, long defaultDuration)
    {
        mTag = parentTag + "_" + TAG;
    ...
    public void addFrame(int bits)
    {
        ...
        if (currTime - mLastTime >= mDefaultDuration)
        {
            if (mLastTime > 0)
            {
                if (bits == 0)
                {
                    UnityEngine.Debug.Log(mTag + "mFps=" + mFps);
                }
                else
                {
                    UnityEngine.Debug.Log(mTag+ "byteSize=" + mBitrate + ":mFps=" + mFps);
                }
            }
            mLastTime = currTime;
            mBitrate = 0;
    ...
    {
        long currTime = System.DateTime.Now.Millisecond;
        if (mFpsList == null)
        {
        ...
        if (currTime - mLastTime >= mDefaultDuration)
        {
            if (mLastTime > 0)
            {
                JLog.Info(mTag, /*Arrays.toString(mFpsList.toArray()) +*/ ",avg=" + getAvg());
                mFpsList.Clear();
            }
            mLastTime = currTime;
            mFps = 0;
