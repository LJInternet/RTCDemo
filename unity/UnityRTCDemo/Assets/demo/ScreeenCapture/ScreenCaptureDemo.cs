...
public class ScreenCaptureDemo : MonoBehaviour
{
    ...
    {
        //Debug.Log("OnCaptureAudioFrame Length " + videoFrame.buffer.Length + " videoFrame.channelCount " + videoFrame.channelCount);
    ...
    public void Start()
    {
        ...
        mRtcEngine.RegisterScreenCaptureVideoFrame(OnScreenCapatureVideoFrame);
        ScreenCaptureParam param = new ScreenCaptureParam();
        param.enableAudio = 1;
        param.enableVideo = 0;
        param.videoFps = 15;
        param.videoWidth = 720;
        param.videoHeight = 1280;
        mRtcEngine.StartScreenCapture(param);
    ...
    public void OnScreenCapatureVideoFrame(CaptureVideoFrame captrueVideo) {
        //JLog.Info("OnScreenCapatureVideoFrame");
        if (captrueVideo.buffer == null || captrueVideo.buffer.Length <= 0) {
            return;
        }
        byte[] msg = captrueVideo.HPmarshall();
        rtmpEngine.WriteVideo(captrueVideo.buffer, captrueVideo.buffer.Length, msg, msg.Length, 1);
    }
    ...
    public void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(30, 150, 100, 80), "开始屏幕采集"))
        {
            IRtcEngine rtcEngine = IRtcEngine.Get();
            if (rtcEngine != null)
            {
                ...
                param.videoWidth = 720;
                param.videoHeight = 1280;
                rtcEngine.StartScreenCapture(param);
            }
        }

        if (GUI.Button(new Rect(30, 230, 100, 80), "停止屏幕采集"))
        {
            IRtcEngine rtcEngine = IRtcEngine.Get();
            if (rtcEngine != null)
            {
